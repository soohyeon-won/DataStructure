...
public class Maze {
	...
	public void findMaze(int current_X, int current_Y){
		...
		*/
		...
		if(!maze[current_X][current_Y].equals("S")){		 
			maze[current_X][current_Y] = ".";		 /* 시작위치가 아니라면 '.'값을 넣어준다. */
			coordinateOfPaths.add("("+(current_X+1)+","+(current_Y+1)+")"+" "); /* 지나간 좌표 저장 */
		}
		/* 길이 있으면 이동하는 부분 */	
		if(current_X>=1 && current_X < row && canMove(current_X-1, current_Y)){		
				findMaze(current_X-1, current_Y); 	 	 	
		}	
		if(current_X>=0 && current_X < row-1 && canMove(current_X+1, current_Y)){ 	//하
				findMaze(current_X+1, current_Y);
		} 
		if(current_Y>=1 && current_Y<col && canMove(current_X, current_Y-1)){ 		//좌  					
	...
	*/	
	...
	/**
	* 지나온 좌표들 (coordinateOfPaths), 이동횟수(distance)를 출력하는 메소드
	*/
